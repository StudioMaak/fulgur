...
These are hand-written notes and cover only the properties someone has written
up. For a **measured** picture of where fulgur diverges from WeasyPrint, see
`docs/css-reference-diff.md`, which is regenerated by `scripts/refdiff/` rather
than maintained by hand. For what each test harness actually asserts — they do
not all mean the same thing by "pass" — see `docs/test-harnesses.md`.
